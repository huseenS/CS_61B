...
public class SList {
  ...
  public void squish() {
	  ...
	  if (isEmpty()) {
		...
		while (next != null) {
			//if next node item equals current node item
  ...
  public void twin() {
	  if (isEmpty()) {
		...
		while(current != null) {
			//add duplicate node that has same item, it's next points to currents next
			current.next = new SListNode(current.item, current.next);
			current = current.next.next; //update current pointer
		}
	}
