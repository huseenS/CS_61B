...
public class HomeWork3 {
  ...
  public static void smoosh(int[] ints) {
    int i =0;
    while (i<ints.length) {
    	//if not negative already-then iterate
		if (ints[i] != -1) {
			//j is next int
    ...
    //shift all -1 elements down
    int x = 0;
    while(x < ints.length) {
    	//if element is -1
    	if (ints[x] == -1) {
			//iterate On x+1th element
    		for (int z = x+1; z < ints.length; z++) {
    			//if next element isn't negative
				if (ints[z] != -1) {
					ints[x] = ints[z]; //set element x equal to z
					ints[z] = -1; //set z equal to -1 shifting all -1 down
					break;
				}
			}
		}
