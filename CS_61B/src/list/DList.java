...
public void insertFront(Object item) {
	  DListNode front;
	  DListNode toInsert = newNode(item, this, null, null);
	  
	  if((front = (DListNode) this.front()).isValidNode()) {
		  //insert node in between head/front node
		  head.next = toInsert;
		  toInsert.prev = head;
		  front.prev = toInsert;
		  toInsert.next = front;	
...
public void insertBack(Object item) {
    DListNode toInsert = new DListNode(item, this, null, null);
    DListNode currentBack;
   
    //if node is valid
    if ((currentBack = (DListNode) this.back()).isValidNode()) {
		toInsert.next = head;
		toInsert.prev = currentBack;
		head.prev = toInsert;
		currentBack.next = toInsert;
		size++;
	} else { //list is empty
		head.next = toInsert;
		head.prev = toInsert;
...
public String toString() {
  ...
  public static void main(String[] argv) {
    ...
    System.out.println("l is a list of 3 elements: " + l.size);
    try {
      ListNode n;
      int i = 1;
